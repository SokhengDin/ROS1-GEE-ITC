...
def robot_velocity(vel_x, vel_y, vel_omega):
    twist = Twist()
    twist.linear.x = vel_x
    twist.linear.y = vel_y
    twist.angular.z = vel_omega
    pub_vel.publish(twist)

def robot_orientation(x, y, theta, base_link: string, child_id: string):
    odom_quat = quaternion_from_euler(0.0, 0.0, theta)
...
def robot_navigation(x, y, theta, vel_x, vel_y, vel_theta, base_link: string, child_id: string):
    odom = Odometry()
...
if __name__ == '__main__':
    rospy.init_node("MPC_DIF", anonymous=True)
...
while MAX_TIME >= time:
        ...
        t.append(time)
        omega.append(omegai)
        vel_x, vel_y, vel_theta = forward_kinematic(ov[0], oomega[0])
        ##robot_orientation(state.x, state.y, state.yaw, "base_link", "odom")
        robot_navigation(state.x, state.y, state.yaw, vel_x, vel_y, vel_theta, "base_link", "odom")
        robot_velocity(vel_x, vel_y, vel_theta)
