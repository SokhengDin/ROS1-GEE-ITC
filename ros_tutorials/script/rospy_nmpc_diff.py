...
def feedback_mpc(pose_msg):
    x = pose_msg.position.x
    y = pose_msg.position.y
    theta = pose_msg.orientatioin.z
    curr_state = np.array([x, y, theta]).reshape((-1, 1))

    return curr_state

pub_vel = rospy.Publisher('cmd_vel', Twist, queue_size=10)
pub_pos = rospy.Publisher('Pose', Pose, queue_size=10)
sub_pos = rospy.Subscriber('Pose', Pose, feedback_mpc)
